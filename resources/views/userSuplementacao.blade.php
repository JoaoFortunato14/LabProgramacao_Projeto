...
@forelse($proteinas as $proteina)
    ...
    <form action="{{ url('/cartStore')}}" method="POST">
        ...
        <img src="{{asset('images/Suplementacao/'.$proteina->imagem)}}" alt="Proteina Whey" style="width:100%">
        <h1>{{$proteina->nome}}</h1>
        <p class="price">{{$proteina->preco}}&euro;</p>
        <p>{{$proteina->info}}</p>
        <input type="hidden" value="{{ $proteina->id }}" id="id" name="id">
        {{ csrf_field() }}
            <input type="hidden" value="{{ $proteina->nome }}" id="name" name="name">
            <input type="hidden" value="{{ $proteina->preco }}" id="price" name="price">
            <input type="hidden" value="{{ $proteina->imagem }}" id="image" name="image">
