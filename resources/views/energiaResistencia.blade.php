...
@forelse($energias as $energia)
    <div class="card" id="section1">
    <form action="{{ url('/cartStore')}}" method="POST">

        <img src="{{asset('images/Suplementacao/'.$energia->imagem)}}" alt="" style="width:100%">
        <h1>{{$energia->nome}}</h1>
        <p class="price">{{$energia->preco}}&euro;</p>
        <p>{{$energia->info}}</p>
        <input type="hidden" value="{{ $energia->id }}" id="id" name="id">
        {{ csrf_field() }}
            <input type="hidden" value="{{ $energia->nome }}" id="name" name="name">
            <input type="hidden" value="{{ $energia->preco }}" id="price" name="price">
            <input type="hidden" value="{{ $energia->imagem }}" id="imagem" name="imagem">
    ...
    @empty
    <h5>No Energy and Resistent Products Found!</h5>
    @endforelse
</div>
