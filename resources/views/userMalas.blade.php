...
@forelse($malas as $mala)
    ...
    <form action="{{ url('/cartStore')}}" method="POST">
        ...
        <img src="{{asset('images/Vestuario/'.$mala->imagem)}}" alt="Proteina Whey" style="width:100%">
        <h1>{{$mala->nome}}</h1>
        <p class="price">{{$mala->preco}}&euro;</p>
        <p>{{$mala->info}}</p>
        <input type="hidden" value="{{ $mala->id }}" id="id" name="id">
        {{ csrf_field() }}
            <input type="hidden" value="{{ $mala->nome }}" id="name" name="name">
            <input type="hidden" value="{{ $mala->preco }}" id="price" name="price">
            <input type="hidden" value="{{ $mala->imagem }}" id="imagem" name="imagem">
