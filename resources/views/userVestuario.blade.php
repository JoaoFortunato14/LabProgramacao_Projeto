...
@forelse($vestuarios as $vestuario)
    ...
    <form action="{{ url('/cartStore')}}" method="POST">
        ...
        <img src="{{asset('images/Vestuario/'.$vestuario->imagem)}}" alt="Proteina Whey" style="width:100%">
        <h1>{{$vestuario->nome}}</h1>
        <p class="price">{{$vestuario->preco}}&euro;</p>
        <p>{{$vestuario->info}}</p>
        <input type="hidden" value="{{ $vestuario->id }}" id="id" name="id">
        {{ csrf_field() }}
            <input type="hidden" value="{{ $vestuario->nome }}" id="name" name="name">
            <input type="hidden" value="{{ $vestuario->preco }}" id="price" name="price">
            <input type="hidden" value="{{ $vestuario->imagem }}" id="imagem" name="imagem">
