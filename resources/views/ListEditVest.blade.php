...
<ul class="list-group">
    ...
    <li class="list-group-item">
        <h5>{{$vestuario->id}} - {{$vestuario->category}} - {{$vestuario->nome}} - {{$vestuario->preco}} - {{$vestuario->info}} - {{$vestuario->stock}} </h5>
        <form action="{{url('/DeleteVest/'.$vestuario->id)}}" method="POST">
            <a class="btn btn-primary" href="{{ url('/EditVest',$vestuario->id) }}">Edit</a>
            @csrf
